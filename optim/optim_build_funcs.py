...
'''
There are two additional hyperparameters that control the optimizer behavior:
...
def build_sgd_multistep(lr=0.01, momentum=0, weight_decay=0, milestones=[60, 100], gama=0.1, max_norm=10.0,
                        no_weight_decay_on_bn=True, **kwargs):
...
@OPTIM_BUILD_FUNCS.register_with_name(module_name='adagrad')
def build_adagrad(lr=0.01, weight_decay=0.001, **kwargs):
    optimizer = torch.optim.Adagrad(kwargs['params'], lr=lr, weight_decay=weight_decay)
    lr_scheduler = None

    return optimizer, lr_scheduler


@OPTIM_BUILD_FUNCS.register_with_name(module_name='adagrad_ca')
def build_adagrad_ca(lr=0.01, weight_decay=0.001, T_max=None, **kwargs):
    optimizer = torch.optim.Adagrad(kwargs['params'], lr=lr, weight_decay=weight_decay)
    if T_max is None:
        T_max = kwargs['args'].epochs
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=T_max)
...
@OPTIM_BUILD_FUNCS.register_with_name(module_name='adam')
def build_adam(lr=0.01, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.001, amsgrad=False, **kwargs):
    optimizer = torch.optim.Adam(kwargs['params'], lr=lr, betas=betas, eps=eps, weight_decay=weight_decay,
                                 amsgrad=amsgrad)
    lr_scheduler = None
    return optimizer, lr_scheduler
...
@OPTIM_BUILD_FUNCS.register_with_name(module_name='adamw')
def build_adamw(lr=0.01, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.001, amsgrad=False, **kwargs):
    optimizer = torch.optim.AdamW(kwargs['params'], lr=lr, betas=betas, eps=eps, weight_decay=weight_decay,
                                  amsgrad=amsgrad)
    lr_scheduler = None
    return optimizer, lr_scheduler


@OPTIM_BUILD_FUNCS.register_with_name(module_name='sgd')
def build_sgd(lr=0.01, momentum=0.9, weight_decay=0.001, **kwargs):
    optimizer = torch.optim.SGD(kwargs['params'], lr=lr, momentum=momentum, weight_decay=weight_decay)
    lr_scheduler = None
    return optimizer, lr_scheduler
