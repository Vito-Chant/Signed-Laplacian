...
from utils.tensorboard import add_train_vs_eval_metric, add_train_metric, add_eval_metric
from torch.utils.tensorboard import SummaryWriter
from engine_gnn import evaluate, train_one_epoch
from models.utils import load_data, calculate_class_weights
from config import abs_path
# import edge feat generator
from utils.gtrick import CommonNeighbors
...
def get_args_parser():
    ...
    parser.add_argument('--config_file', default='./config/config.yml', type=str, help='config dir')
...
def train(local_rank, args):
    ...
    if args.optim_no_weight_decay_on_bn:
        params_only_bn, params_wo_bn = misc.separate_bn_paras(model_without_module)
        optimizer, lr_scheduler = optim_build_func(
            params=[{'params': params_wo_bn}, {'params': params_only_bn, 'weight_decay': 0.0}], args=args)
    else:
        optimizer, lr_scheduler = optim_build_func(
    ...
    if args.dataset is not None:
        ...
        if args.parallel_mode == 'ddp':
            ...
            if criterion_eval is not None:
                ...
                batch_sampler_eval = torch.utils.data.BatchSampler(sampler_eval, args.batch_size_per_gpu,
                                                                   drop_last=False)
        else:
    ...
    ex = None
    if args.model == 'aict' or args.model == 'slgnn_tdhnn':
        num_nodes, num_edges, train_pos, train_neg, train_pos_dir, train_neg_dir, test_pos_dir, test_neg_dir, num_feats, input_features = load_data(
            args, abs_path, seed=None)
        args.class_weights = calculate_class_weights(num_edges[0], num_edges[1])

        if args.model_common_neighbors:
            ef = CommonNeighbors(torch.cat([train_pos, test_pos_dir], dim=1), batch_size=1024,
                                 sparse_sizes=[num_nodes, num_nodes])
            ex = {}
            ex['train'] = torch.cat([ef(edges=train_pos.t()), ef(edges=train_neg.t())], dim=0).to(device)
            ex['test'] = torch.cat([ef(edges=test_pos_dir.t()), ef(edges=test_neg_dir.t())], dim=0).to(device)
            # ex['train']['pos'] = ef(edges=train_pos).to(device)
            # ex['train']['neg'] = ef(edges=train_neg).to(device)
            # ex['test']['pos'] = ef(edges=test_pos_dir).to(device)
            # ex['test']['neg'] = ef(edges=test_neg_dir).to(device)

        if args.device == 'cuda':
            node_features = torch.FloatTensor(input_features).cuda()
            ys_float = torch.LongTensor([1] * train_pos.shape[1] + [0] * train_neg.shape[1]).cuda()
            train_pos = train_pos.cuda()
            train_neg = train_neg.cuda()
        else:
            node_features = torch.FloatTensor(input_features)
            ys_float = torch.LongTensor([1] * train_pos.shape[1] + [0] * train_neg.shape[1])
        # print(node_features.shape, train_pos.shape, train_neg.shape)
    else:
        node_features = None
        train_pos = None
        train_neg = None
    ...
    if args.model_nr > 0:
        import random
        num_labels = ys_float.size(0)
        num_to_flip = int(num_labels * args.model_nr)
        # 随机选择需要反转的标签的索引
        indices_to_flip = random.sample(range(num_labels), num_to_flip)
        # 反转选中的标签
        for index in indices_to_flip:
            ys_float[index] = 1 - ys_float[index]
    ...
    if args.eval and criterion_eval is not None:
        ...
        eval_stats = evaluate(model, criterion_eval, data_loader_eval, epoch=-1, args=args,
                              ...
                              wandb_run=wandb_run, node_features=node_features, train_pos=train_pos,
                              train_neg=train_neg, ys_float=ys_float, test_pos_dir=test_pos_dir,
                              test_neg_dir=test_neg_dir, ex=ex['test'] if ex is not None else None)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(model, criterion_train, optimizer, data_loader_train, epoch, args=args,
                                      ...
                                      node_features=node_features, train_pos=train_pos, train_neg=train_neg,
                                      ys_float=ys_float, ex=ex['train'] if ex is not None else None)
        ...
        if misc.is_main_process():
            with open(os.path.join(args.output_dir, 'log.txt'), 'a') as f:
                ...
                add_eval_metric(tensorboard_writer, eval_stats, epoch)
    ...
    return best_metric_holder.best_res, best_stats
