...
class Registry(object):
    ...
    def register(self, module_build_function, module_name=None, artifact=None, force=False):
        ...
        #     self._artifact_dict[module_name] = {}
        #     os.environ['WANDB_SILENT'] = 'true'
        #     with wandb.init(project=__file__.split('/')[-3], job_type='registry', name=module_name) as run:
        #         for a in artifact:
        #             artifact = wandb.Artifact(name=a['name'], type=a['type'])
        #             artifact.add_file(os.path.abspath(sys.modules[a['obj'].__module__].__file__))
        #             artifact = run.log_artifact(artifact)
        #             artifact.wait()
        #             self._artifact_dict[module_name][a['name']] = (artifact.qualified_name, a['type'])
